...
from model_gt import DM2FNetv8_woPhy,Discriminator
...
def parse_args():
    ...
    parser.add_argument('--ckpt-path', default='./ckpt_ohaze', help='checkpoint path')
    parser.add_argument(
        '--exp-name',
        default='O-Haze',
...
cfgs = {
    ...
    'weight_decay': 2e-5,
    'momentum': 0.9,
    'snapshot': 'iter_80000_loss_0.04029_lr_0.000000',
    'val_freq': 2000,
    'crop_size': 512,
}
...
def main():
    net = DM2FNetv8_woPhy().cuda().train()
    ...
    while curr_iter <= cfgs['iter_num']:
        ...
        for data in train_loader:
            ...
            with amp.autocast():
                ...
                loss = gan_loss+loss_x_jf + loss_x_j1 + loss_x_j2 + loss_x_j3 + loss_x_j4
