...
def make_dataset_hazerd(root,dis):
    items = []
    for img_name in os.listdir(os.path.join(root,'img')):
        idx0, idx1, ato = os.path.splitext(img_name)[0].split('_')
        gt = os.path.join(root,'img',img_name)
        haze = os.path.join(root, 'simu', idx0+'_'+idx1+'_'+dis+'.jpg')
...
class HazeRD(data.Dataset):
    def __init__(self, root,dist='1000',resize=512):
        self.root = root
        self.img_name_list = make_dataset_hazerd(root,dist)
...
######LJH'S TEST#####
class HX_SotsDataset(data.Dataset):
    def __init__(self, root, mode=None):
        self.root = root
        self.imgs = make_dataset(root)
        self.mode = mode
...
class HX_ItsDataset(data.Dataset):
    """
    For RESIDE Indoor
    """
    ...
    def __getitem__(self, index):
        ...
        return haze, trans, gt_ato, haze-gt, name
