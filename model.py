...
from dehazeformer_model import dehazeformer_t 
#baseline模型
...
#dm2f_model_+dehazetransformer
class Trans_DM2FNet(Base):
    def __init__(self, num_features=128, arch='resnext101_32x8d'):
        super(Trans_DM2FNet, self).__init__()
        self.num_features = num_features
        ...
        self.net2=dehazeformer_t()
        #self.net3=dehazeformer_t()
        self.down1 = nn.Sequential(
            nn.Conv2d(256, num_features, kernel_size=1), nn.SELU()
        )
        self.down2 = nn.Sequential(
...
#dm2f_model_change the resnext to dehazetransformer_block
class dehazeformer_DM2FNet(Base):
    def __init__(self, num_features=48, arch='resnext101_32x8d'):
        super(dehazeformer_DM2FNet, self).__init__()
        self.num_features = num_features
        ...
        self.backbone = backbone
        #add
        self.net1=dehazeformer_t()
        #self.rescale=
        #end
        #self.net3=dehazeformer_t()
        self.down1 = nn.Sequential(
            nn.Conv2d(48, num_features, kernel_size=1), nn.SELU()
        )
        self.down2 = nn.Sequential(
            nn.Conv2d(48, num_features, kernel_size=1), nn.SELU()
        )
        self.down3 = nn.Sequential(
            nn.Conv2d(24, num_features, kernel_size=1), nn.SELU()
        )
        self.down4 = nn.Sequential(
            nn.Conv2d(4, num_features, kernel_size=1), nn.SELU()
        )
        ...
        self.back_size=nn.Sequential(
            nn.Conv2d(2048,1024,kernel_size=3,padding=1),nn.SELU(),
            nn.Conv2d(1024,512,kernel_size=3,padding=1),nn.SELU(),
            nn.Conv2d(512,128,kernel_size=3,padding=1),nn.SELU(),
    ...
    def forward(self, x0, x0_hd=None):
        ...
        x = backbone.layer4(x)
        x=self.back_size(x)
        x=F.upsample(x, size=x0.size()[2:], mode='bilinear')

        net=self.net1
        layer0 = net.patch_embed(x)
        layer0 = net.layer1(layer0)
        skip1=layer0

        layer1 = net.patch_merge1(layer0)
        layer1=net.layer2(layer1)
        skip2=layer1

        layer2=net.patch_merge2(layer1)
        layer2 = net.layer3(layer2)
        layer2=net.patch_split1(layer2)

        layer3 = net.fusion1([layer2,net.skip2(skip2)])+layer2
        layer3=net.layer4(layer3)
        layer3=net.patch_split2(layer3)
        

        layer4 = net.fusion2([layer3,net.skip1(skip1)])+layer3
        layer4=net.layer5(layer4)
        layer4=net.patch_unembed(layer4)
